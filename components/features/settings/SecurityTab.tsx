...
function PasswordSection({ hasPassword }: { hasPassword: boolean }) {
  ...
  if (!passwordSet) {
    ...
    return (
      ...
      >
        {error && <div role="alert" className="mb-3 rounded border border-red-500/20 bg-red-500/10 px-3 py-2 text-base text-red-400">{error}</div>}
        {success && <div className="mb-3 rounded border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-base text-emerald-400">Password set successfully! You can now sign in with email too.</div>}
        <form action={handleSetPassword} className="space-y-3 max-w-sm">
          <div className="space-y-1.5">
            <Label htmlFor="newPassword" className="text-base text-muted-foreground">Create a password</Label>
  ...
  return (
    <SettingsSection title="Change Password" description="Update your password regularly for security.">
      {error && <div role="alert" className="mb-3 rounded border border-red-500/20 bg-red-500/10 px-3 py-2 text-base text-red-400">{error}</div>}
      {success && <div className="mb-3 rounded border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-base text-emerald-400">Password changed successfully</div>}
      <form action={handleChangePassword} className="space-y-3 max-w-sm">
        <div className="space-y-1.5">
          <Label htmlFor="currentPassword" className="text-base text-muted-foreground">Current password</Label>
...
function EmailChangeSection({ currentEmail }: { currentEmail: string }) {
  ...
  return (
    <SettingsSection title="Email Address" description={`Current: ${currentEmail}`}>
      {error && <div role="alert" className="mb-3 rounded border border-red-500/20 bg-red-500/10 px-3 py-2 text-base text-red-400">{error}</div>}
      {success && <div className="mb-3 rounded border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-base text-emerald-400">Email updated</div>}
      <form action={handleSubmit} className="space-y-3 max-w-sm">
        <div className="space-y-1.5">
          <Label htmlFor="newEmail" className="text-base text-muted-foreground">New email</Label>
          <input id="newEmail" name="newEmail" type="email" required
            className="w-full rounded bg-[var(--input)] border border-[var(--border)] px-3 py-2 text-base text-foreground outline-none focus:border-[color-mix(in srgb,var(--foreground) 30%,transparent)]" />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="emailPassword" className="text-base text-muted-foreground">Confirm password</Label>
  ...
  return (
    <SettingsSection title="Active Sessions" description="Manage devices where you're signed in.">
      <div className="space-y-2">
        {sessions.map((s) => (
          <div key={s.id} className="flex items-center gap-3 rounded-lg border border-[var(--border)] p-3 text-base">
            <DeviceIcon os={s.os} />
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="font-medium text-foreground">{s.device}</span>
                {s.current && <span className="rounded bg-emerald-500/20 px-1.5 py-0.5 text-sm font-bold text-emerald-400">Current</span>}
              </div>
      ...
      {sessions.filter((s) => !s.current).length > 0 && (
        <button onClick={handleRevokeAll} disabled={isPending}
          className="mt-3 flex items-center gap-1.5 text-base text-red-400 hover:text-red-300">
